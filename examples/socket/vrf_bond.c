...
int main (int argc, char *argv[])
{
	...
	//snprintf (ifr.ifr_ifru.ifru_slave, sizeof (ifr.ifr_ifru.ifru_slave), argv[2]);
	snprintf (ifr.ifr_slave, sizeof (ifr.ifr_slave), "%s", argv[2]);

	if (ioctl (sfd, SIOCBONDENSLAVE, &ifr) < 0) {
		printf("Error: Bonding failed : %d %s\n", errno, strerror(errno));
		exit(1);
	}
#endif
	...
	req.i.ifi_index = if_nametoindex(argv[2]);
	printf("IFI of slave %s : %d\n", argv[2], req.i.ifi_index);
